...
class ValidationUtils:
    ...
    @staticmethod
    def expand_children_with_groups(children: List[str], schema_info: ISchemaInfo) -> List[str]:
        """
            ...
            if child.startswith("GROUP:"):
                ...
                if group_def:
                    expanded_children.extend(
                        ValidationUtils._expand_group_recursively(group_def, schema_info)
                    )
                else:
    ...
    @staticmethod
    def _expand_group_recursively(group_def: IGroupDefinition, schema_info: ISchemaInfo) -> List[str]:
        """
        ...
        for child in group_def.children:
            if child.startswith("GROUP:"):
                ...
                if nested_group_def:
                    elements.extend(
                        ValidationUtils._expand_group_recursively(nested_group_def, schema_info)
                    )
            else:
